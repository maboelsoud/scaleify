...
export const fetchStoreFromFirebase = async (
  CallSid: string,
...
export const saveBusinessToFirebase = async (
  business: Business,
): Promise<Business | void> => {
  const res = await getBusiness()
    .doc(business.primaryEmail)
...
export const fetchBusinessFromFirebase = async (
  email: string,
): Promise<Business | void> => {
...
export const fetchBusinessByToNumber = async (
  toNumber: string,
): Promise<Business | void> => {
  const snapshot = await getBusiness()
    .where("twilioIncomingPhoneNumber", "==", toNumber)
    .get();
  if (snapshot.empty) {
    console.log("No matching documents.");
    return;
...
export const fetchOperatorFromFirebase = async (
  toNumber: string,
): Promise<string | void> => {
...
export const fetchBusinessInfoFromFirebase = async (
  toNumber: string,
  infoSegment: string,
): Promise<string | void> => {
  const fetchedBusiness = await fetchBusinessByToNumber(toNumber);
...
export const executeCommand = async (
  commandFn: string,
): Promise<{ success: boolean; text: string; error?: string } | void> => {
...
export const getResponseFromLLM = async (
  ...
  if (Array.isArray(convoHistory)) {
    const lastMessage =
      convoHistory[convoHistory.length - 1]?.systemToMachine || "";
