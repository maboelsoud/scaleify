...
export type EventType =
  ...
  | {
      ...
      payload: { twilioParams: TwilioVoiceWebhookParams };
    }
  | {
      type: "PROCESSING_LLM";
      payload: { who: "customer" | "system"; message: string };
    }
  | {
      type: "LLM_TEXT_RESPONSE";
      payload: { message: string; expectReply: boolean };
    }
  ...
  | {
      type: `ERROR_NO_CONVO`;
      payload: { error: string; twilioParams: TwilioVoiceWebhookParams };
    }
...
export async function storelessDispatch({
  event,
  emit,
}: {
  ...
  if (nextEvent) {
    if (updatedStore) {
      return storeFullDispatch({
        store: updatedStore,
        event: nextEvent as storeFullEvent,
        emit,
      });
    } else {
...
export async function storeFullDispatch({
  store,
  event,
  emit,
}: {
