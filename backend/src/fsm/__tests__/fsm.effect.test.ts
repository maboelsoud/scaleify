...
describe("Effects", () => {
  test("CREATED effect produces correct next state", async () => {
    ...
    const mockStore = updateStoreState(
      createStore(twilioParams),
      "PROCESSING_GREETING",
    );
  ...
  test("FETCHING_CUSTOMER_INPUT effect produces correct next state", async () => {
    ...
    const secondResult = await Effects["FETCHING_CUSTOMER_INPUT"](
      initialStore,
      {
        type: "FETCHING_CUSTOMER_INPUT",
        payload: {
    ...
    const secondExpectedStore = updateStoreState(
      initialStore,
      "NO_CUSTOMER_INPUT",
    );
  ...
  test("NO_CUSTOMER_INPUT effect produces correct next state", async () => {
    ...
    expect(secondResult.nextEvent?.payload.message).toEqual(
      SYSTEM_MESSAGES.noInputTwice,
    );
  ...
  test("PROCESSING_LLM effect produces correct next state", async () => {
    ...
    expect(secondResult.updatedStore.messages[1].systemToMachine).toEqual(
      "second test",
    );
  ...
  test("SENDING_RESPONSE effect produces correct next state", async () => {
    ...
    const result = await Effects["SENDING_RESPONSE"](initialStore, {
      ...
      payload: { expectReply: true, message: "this is a test message" },
    });
    ...
    const secondResult = await Effects["SENDING_RESPONSE"](initialStore, {
      type: "SENDING_RESPONSE",
      payload: { expectReply: false, message: "this is a test message" },
    });
